...
-- CreateEnum
CREATE TYPE "ApplicationStatus" AS ENUM ('RECEIVED', 'UNDER_REVIEW', 'INTERVIEWING', 'OFFERED', 'HIRED', 'REJECTED', 'WITHDRAWN');
...
-- CheckConstraint: salarios no negativos y rango coherente (manual, no soportado por Prisma schema)
ALTER TABLE "Position" ADD CONSTRAINT "Position_salary_nonnegative_check"
    CHECK (("salaryMin" IS NULL OR "salaryMin" >= 0) AND ("salaryMax" IS NULL OR "salaryMax" >= 0));
ALTER TABLE "Position" ADD CONSTRAINT "Position_salary_range_check"
    CHECK ("salaryMin" IS NULL OR "salaryMax" IS NULL OR "salaryMin" <= "salaryMax");

-- CheckConstraint: puntuación de entrevista acotada 0-100
ALTER TABLE "Interview" ADD CONSTRAINT "Interview_score_range_check"
    CHECK ("score" IS NULL OR ("score" >= 0 AND "score" <= 100));

-- CheckConstraint: orden de paso no negativo
ALTER TABLE "InterviewStep" ADD CONSTRAINT "InterviewStep_orderIndex_nonnegative_check"
    CHECK ("orderIndex" >= 0);
